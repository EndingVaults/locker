...
mt.index = newcclosure(function(t, k)
    if (t:IsA("Mouse") and (k == "Hit")) then
        ...
        if (SilentAim.checkSilentAim()) then
            if (CPlayer.Character:FindFirstChild("RandomPart")) then
                return {p=(CPlayer.Character.RandomPart.CFrame.p+(CPlayer.Character.RandomPart.Velocity*accomidationfactor))}
            end
        end
    end
